...
class QordobaAPI {
  ...
  const SANDBOX_URL = 'http://app.qordobadev.com/api';
  const PRODUCTION_URL = 'https://api.qordoba.com/api';
  ...
  /**
   * upload file to Qordoba service
   *
   * @param string $path
   * @param array $params
   * @return string
  ...
  public function uploadFile($path, $params = array()) {
    ...
    # TODO: figure out a way to workaround file ID
    $handle = "projects/$this->_projectId/files";
    return $this->sendRequest($handle, $params, HttpClient::REQUEST_TYPE_POST, true);
  }

  /**
   * upload content to Qordoba service
   *
  ...
  /**
   * download translated content from Qordoba Service
   *
  ...
  public function downloadFile($fileUri, $locale, $params = array()) {
    $handle = "projects/$this->_projectId/languages/$locale/files/$fileUri";
//    $sendData = array_replace_recursive(array(
//        'fileUri' => $fileUri,
//        'locale' => $locale
//    );
    $sendData = $params;
    return $this->sendRequest($handle, $sendData, HttpClient::REQUEST_TYPE_GET);
  }
  ...
  /**
   * remove uploaded files from Qordoba Service
   *
   * @param string $fileUri
   * @return string
   */
  ...
  /**
   * send request to Qordoba Service
   *
   * @param string $uri
...
//    $request = array_replace_recursive($data, $requestData);
    ...
    $connection->addHeader("X-AUTH-TOKEN", $this->_apiKey)
        ->addHeader("user_key", $this->_apiKey)
        ->setMethod($method)
