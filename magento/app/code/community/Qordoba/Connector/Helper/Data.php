 ...
 * @author Itdelight
 */
class Qordoba_Connector_Helper_Data
...
{   
    ...
    public function prepareLocalesForSave($stores){
        if (!is_array($stores)){
            Mage::log ('uncorrect data for stores', null, 'QordobaLog.log');
            return false;
        }
    ...
    public function getFileUri($identifier){
        $timetamp = time();
        $fileType = Mage::getStoreConfig('qordoba/settings/file_type');
    ...
    /**
     * Return Id of default source view for Qordoba module
     * @return int
     * @deprecated since version 0.2.1
     */
    public function getSourceStoreView($store_group_id) {
        
        $defaultStoreId = Mage::getStoreConfig('qordoba/settings/source_store_view');
...
//        return (bool)(preg_match('/^\w{8}(?:-\w{4}){3}-\w{12}$/', $val));
    ...
    public function log($message, $level = Zend_log::INFO) {
        ...
        $log_file = Mage::getStoreConfig('dev/qordoba/log_file');
        $log_level = Mage::getStoreConfig('dev/qordoba/log_level');
        $writeLog = false;
        
        if(!$log_file) {
            $log_file = 'qordoba.log';
        }
        ...
        if($writeLog) {
            ...
            $mock_data = Mage::getStoreConfig('dev/qordoba/mock_data');
    ...
    public function changeStatus($requestType, $id, $entityGlobalType, $keyField) {
        ...
        try {
          ...
          $result['message'] = Mage::helper('connector')
                  ->__('Application internal error. Please see qordoba log for details');
