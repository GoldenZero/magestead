 ...
 * @author Qordoba
 */
class Qordoba_Connector_Adminhtml_InstanceController
    extends Mage_Adminhtml_Controller_Action
{
    ...
    /**
     * Bulk upload single content to Qordoba
    ...
    public function confirmAction() {
        ...
        try {
            ...
            $contentTypeTitle = strtolower($contentModelClass->getTitle());
        ...
        //]] submition validation
            ...
            $message = '';
            if($countAddedItem > 1) {
                $message = Mage::helper('connector')->__('%d %s uploaded to Qordoba for translation.',
                    $countAddedItem, $contentTypeTitle);
            } elseif($countAddedItem == 1) {
                $message = Mage::helper('connector')->__('1 %s uploaded to Qordoba for translation.',
                    $contentTypeTitleSingle);
            } if(!$countAddedItem && $alreadyAddedItem) {
                $message = Mage::helper('connector')->__('These %s already exist in your Qordoba project.',
                    $contentTypeTitle);
            }
            ...
            if($countAddedItem && $alreadyAddedItem) {
                $message = Mage::helper('connector')->__('%d %s uploaded to Qordoba for translation. %d %s already exist in your Qordoba project and %s not sent.',
                    $countAddedItem, $countAddedItem == 1 ? $contentTypeTitleSingle : $contentTypeTitle,
                    $alreadyAddedItem,  $alreadyAddedItem == 1 ? $contentTypeTitleSingle : $contentTypeTitle,
                    $alreadyAddedItem == 1 ? 'was' : 'were');
                Mage::getSingleton('adminhtml/session')->addSuccess($message);
                Mage::helper('connector')->log($message, Zend_log::INFO);
            }

            if(($countAddedItem + $alreadyAddedItem) != (count($data['content']) * $localesCounter)) {
                $failedAddedItem = (count($data['content']) * $localesCounter) - ($countAddedItem + $alreadyAddedItem);

                $message = Mage::helper('connector')
                    ->__('%d %s uploaded to Qordoba for translation. %d %s already exist in your Qordoba project and %s not sent. %d %s failed to send.',
                        $countAddedItem, $countAddedItem == 1 ? $contentTypeTitleSingle : $contentTypeTitle,
                        $alreadyAddedItem,  $alreadyAddedItem == 1 ? $contentTypeTitleSingle : $contentTypeTitle,
                        $alreadyAddedItem == 1 ? 'was' : 'were',
                        $failedAddedItem, $failedAddedItem == 1 ? $contentTypeTitleSingle : $contentTypeTitle);
                Mage::getSingleton('adminhtml/session')->addError($message);
                Mage::helper('connector')->log($message, Zend_log::ERR);
            }
            //]] submition report

            Mage::dispatchEvent('qordoba_push_to_queue_after',
                array(
