...
class Qordoba_Connector_Model_Translator
    extends Mage_Core_Model_Abstract
{
    
    /**
     * xml file type for Qordoba Api while uploading files
     */
    const FILE_TYPE_XML = "xml";
    
    /**
     * CSV file type for Qordoba Api while uploading files
     */
    const FILE_TYPE_CSV = "csv";
    
    /**
     * test mode Qordoba API
     */
    const TEST_MODE = 'SANDBOX';
    
    /**
     * production mode Qordoba API
     */
     ...
     * @var null | QordobaAPI
     */
    protected $_qordobaApiModel = null;
    
    /**
     *
     * @var string
     */
    protected $_code = 'qordoba';
    ...
    /**
     * List of Qordoba API lib objects
     * @var array 
     */
    protected $_qordobaObjectPool = null;
    
    /**
     * List of Qordoba API data
     * @var array 
     */
     ...
     * @var string 
     */
    protected $_objectPoolKeyPrefix = 'qordoba_';
    ...
    public function __construct($options = array()) {
        ...
        if($options['project_id']) {
            $this->initQordobaApiInstance($options['project_id']);
        }
    ...
    public function uploadTranslateData($path, $fileUri, $params = array()){
        ...
        $upload_params->setLocalesToApprove($locales)
         ...
         $responce = $this->_qordobaApiModel
     ...
     * @return boolean|\QordobaAPI
     * @throws Varien_Exception
     */
    protected function initQordobaApiInstance($mage_project_id) {
        ...
        $qordobaObjectPoolKey = $this->getPoolObjectKey($mage_project_id);
        
        $qordobaObject = Mage::objects($qordobaObjectPoolKey);
        
        if($qordobaObject instanceof Qordoba_Connector_Model_QordobaAPI) {
            $this->_qordobaApiModel = $qordobaObject;
            return $qordobaObject;
        } elseif( ($qordobaObject instanceof Qordoba_Connector_Model_QordobaAPI)===false && isset($qordobaObject)) {
            throw new Varien_Exception('Qordoba Object with key ' . $qordobaObjectPoolKey . ' has unexpected type.');
        }
        
        $qordobaObject = $this->getApi(null,
                                         $this->_projectsDataPool[$mage_project_id]['apiKey'],
                                         $this->_projectsDataPool[$mage_project_id]['projectId'],
                                         $this->_mode()
                                        );
        
        Mage::objects()->save($qordobaObject, $qordobaObjectPoolKey);
     ...
     * @return QordobaAPI
     */
    protected function setQordobaApiInstance($options) {
    ...
    private function getPoolObjectKey($mage_project_id) {
     ...
     /**
     * @param int $mage_project_id
     * @return \QordobaApi_lib_QordobaAPI
     */
    public function getQordobaApiInstance($mage_project_id) {
        
        $qordobaObjectPoolKey = $this->getPoolObjectKey($mage_project_id);
        $qordobaObject = Mage::objects($qordobaObjectPoolKey);
        
        if (is_null($qordobaObject)) {
            return $qordobaObject;
        } else {
            return $this->initQordobaApiInstance($mage_project_id);
        }
    }
    ...
    protected function _getMode(){
        if (!is_null(Mage::getStoreConfig('dev/qordoba/mode'))){
           if (Mage::getStoreConfig('dev/qordoba/mode') == false){
               return false;
           }
        }
       return true;
    ...
    public function createFilesInProcess($locale, $data){        
        ...
        $process->setData($data)
                ...
                ->setStoreId($stores)
                ->setPercent(1.00)
                ->setStatus(Qordoba_Connector_Model_Content::CONTENT_STATUS_PROCESS);
    ...
    public function updateContentStatus($fileUri, $locale){
        return $this->_qordobaApiModel->getStatus($fileUri, $locale);
    }
    ...
    /**
     * upload new content to Qordoba for translation. Check result. In case of
     ...
     * 
     * @param Qordoba_Connector_Model_Content $contentModel
        ...
        if ($response && $this->convertResponse($response)){
            ...
            if ($result->response->code == 'SUCCESS') {                
                Mage::getSingleton('adminhtml/session')
                       ->addSuccess(
                               Mage::helper('connector')->__("Content has been successfully sent to Qordoba")
                               );
     ...
     * 
     * @param string $key
     * @return \Qordoba_Connector_Model_Translator
     */
    ...
    public function getApi() {
        ...
        return Mage::getModel('connector/QordobaAPI', $args);
    }
}
