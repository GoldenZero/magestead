 ...
 * @author Qordoba
 */
class Qordoba_Connector_Model_Service_Observer
{
    public function prepareAttributesOptions(Varien_Event_Observer $object) {
        ...
        if($cmsContentModel instanceof Qordoba_Connector_Model_Content_EavEntityInterface) {
    ...
    public function updateTranslatedContentId(Varien_Event_Observer $object) {
        ...
        if (is_object($newContent) && $newContent->getId()) {
            ...
            try {
                ...
                $logMessage = sprintf('Qordoba saves the original xml file for entity type = %s, id = %s. Locale: %s. Project: %s',
                        $item->getModelClass(),
                        $newContent->getId(),
                        $item['locale'],
                        $item['project_code']
