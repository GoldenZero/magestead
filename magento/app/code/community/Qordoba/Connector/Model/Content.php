 ...
 * @author Qordoba
 */
class Qordoba_Connector_Model_Content
    extends Mage_Core_Model_Abstract
{
     ...
     * @return Qordoba_Connector_Model_Resource_Content_Collection
     */
    ...
    public function getNewItems($project_id = 0, $content_id = 0){
        $status = Qordoba_Connector_Model_Content::CONTENT_STATUS_NEW;
    ...
    public function checkReadyForDownload($status, $locale_name) {
        ...
        if($status == Qordoba_Connector_Model_Content::CONTENT_STATUS_NEW) {
            $errors[] = Mage::helper('connector')->__("Sorry, the entity hasn't being translated yet. Locale [%s]", $locale_name);
        } elseif($status == Qordoba_Connector_Model_Content::CONTENT_STATUS_COMPLETED) {
